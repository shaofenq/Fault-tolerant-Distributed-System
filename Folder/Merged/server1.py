...
def threaded_client(connection, server_name):
    ...
    if(client_name != 'LFD1' and request_message != ''):
        logging.info(f"{client_name} Request: {request_message}")

    reply_message = request_message

    global state
    global isPrimary
    global isActive
    if (isActive == True) or (isActive == False and isPrimary == True):
        ...
        if (client_name == 'C1'):
            ...
            print("#########")
            print("Process client's request, change state from {} to {}. for client {}".format(state[0], request_message, client_name))
        ...
        if (client_name == 'C2'):
            ...
            current_time = datetime.datetime.now()
            print("#########")
            print("Process client's request, change state from {} to {}. for client {}".format(state[1], request_message, client_name))
            # server state after change
        ...
        if (client_name == 'C3'):
            ...
            current_time = datetime.datetime.now()
            print("#########")
            print("Process client's request, change state from {} to {}. for client {}".format(state[2], request_message, client_name))
            # server state after change
    ...
    if client_name == 'RM':
        if request_message == 'checkpoint':
            ...
            if reply_list[0] == 'status':
                global status
                isActive = True if reply_list[1] == "active" else False
                status = reply_list[1:]
                if status[1] == 'primary':
                    isPrimary = True
    ...
    print(status)
    print("Active" if isActive else "Passive")
    print(state)

    # send and print receipt message to client/LFD 
    current_time = datetime.datetime.now()
    connection.send(reply_message.encode())
    #print("[{}] Sending: <{}, {}, {}, reply>".format(str(current_time), client_name, server_name, reply_message))

    if(client_name != 'LFD1' and reply_message != ''):
        logging.info(f"{client_name} Reply: {reply_message}")
