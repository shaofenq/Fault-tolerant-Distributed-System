...
def update_status_from_membership():
    global status
    global membership
    status[1] = 'alive' if 'S1' in membership else 'died'
    status[2] = 'alive' if 'S2' in membership else 'died'
    status[3] = 'alive' if 'S3' in membership else 'died'
...
def GFD_client_connection(connection):
    while True:
        ...
        if request_message == "change":
            status[0] = "active" if status[0] == "passive" else "passive"
